...
public class UserServiceImpl implements UserService {
    ...
    public List<UserDTO> findAllByName(String name) {
        ...
        List<UserDTO> userDTOS = userRepository.findAllByName(name)
                .stream()
                .map(mappingService::mapToDto)
                .toList();
        if (userDTOS.isEmpty()) {
            log.error("[UserService.findByName()] User for given firstname:[{}] not found", name);
            throw new UserNotFoundException(String.format("User not found (name:[%s])", name));
        } else {
            log.debug("[UserService.findByName()] User received from database: [{}], for name:[{}]", userDTOS, name);
            return userDTOS;
        }
    }
